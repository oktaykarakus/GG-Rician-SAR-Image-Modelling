function ppdf = GGRicianAmpPdf(x_range, Alpha, Delta, Gamma)
%% The function for calculating the pdf of GG-Rician amplitude.
%
%  The GG-Rician probability density function for modeling an amplitude signal is given below [1]
%
% $$f(r|\alpha, \gamma, \delta) = \frac{\alpha^2 r}{4\gamma^2\Gamma^2(\frac{1}{\alpha})}
% \int_0^{2\pi} \exp\left(-\frac{|r\cos\theta - \delta|^{\alpha} + |r\sin\theta - \delta|^{\alpha}}{\gamma^{\alpha}} \right) d\theta,$$
% 
% INPUTS:
...
for i = 1:length(x_range)
    funTheta  = @(theta) exp(-(1/(Gamma^Alpha))*(abs(x_range(i).*cos(theta) - Delta).^Alpha + abs(x_range(i).*sin(theta) - Delta).^Alpha));
    ppdf(i) = ((Alpha^2)*x_range(i)./(4*(gamma(1/Alpha)^2)*(Gamma^2))).*integral(funTheta, 0, 2*pi);
end
